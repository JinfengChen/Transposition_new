...
Combine RelocaTE results of 275 RILs into one gff, which will used to do analysis on unique mPing and excision analysis.
    '''
    print message
...
def main():
    ...
    #RIL275_RelocaTE/RelocaTEi_GN/repeat/results/ALL.all_nonref_insert.gff 
    data    = defaultdict(lambda : list())
    output0 = '%s.CombinedGFF.characterized.gff' %(args.input) #confident non_ref only
    output1 = '%s.CombinedGFF.ALL.gff' %(args.input)           #all non_ref insertion, including insufficent data
    output2 = '%s.CombinedGFF.Shared.gff' %(args.input)        #share insertion between ref and strains
    output3 = '%s.CombinedGFF.Non_ref.gff' %(args.input)       #should be same with confident non_ref only
    output4 = '%s.CombinedGFF.Ref_only.gff' %(args.input)      #Reference only not present in strains 
    ofile0  = open(output0, 'w')
    ofile1  = open(output1, 'w')
    ofile2  = open(output2, 'w')
    ofile3  = open(output3, 'w')
    ofile4  = open(output4, 'w')
    dirs    = glob.glob('%s/RelocaTE_*' %(args.input))
    for d in dirs:
        ril    = re.split(r'\_', os.path.split(d)[1])[1]
        ril_id = re.sub(r'\D+', '', ril)
        all_insert  = '%s/mping/results/%s.mping.all_nonref.txt' %(d, ril)
        os.system('perl /rhome/cjinfeng/BigData/00.RD/RILs/Transpostion/bin/relocate2gff.pl --mping %s' %(all_insert))
        gff         = '%s/mping/results/%s.mping.all_nonref.gff' %(d, ril)
        gff_class   = '%s/mping/results/%s.mping.all_nonref.characTErized.gff' %(d, ril)
        gff_nonref  = '%s/mping/results/%s.mping.all_inserts.Non_Ref.gff' %(d, ril)
        gff_shared  = '%s/mping/results/%s.mping.all_inserts.Shared.gff' %(d, ril)
        gff_refonly = '%s/mping/results/%s.mping.all_inserts.Ref_Only.gff' %(d, ril)
        parse_gff(gff, ril_id, ofile1)
        parse_gff(gff_class, ril_id, ofile0)
        parse_gff(gff_shared, ril_id, ofile2)
        parse_gff(gff_nonref, ril_id, ofile3)
        parse_gff(gff_refonly, ril_id, ofile4)
    ofile0.close()
    ofile1.close()
    ofile2.close()
    ofile3.close()
