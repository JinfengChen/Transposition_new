...
def runjob(script, lines):
    cmd = 'perl /rhome/cjinfeng/BigData/software/bin/qsub-pbs.pl --maxjob 10 --lines %s --interval 120 --resource nodes=1:ppn=16,walltime=100:00:00,mem=30G --convert no %s' %(lines, script)
    #print cmd 
    os.system(cmd)
...
def main():
    ...
    for read_dir in sorted(read_dirs):
        ...
        if not os.path.exists(outdir):
            ...
            relocaTE = '%s --mate_1_id _p1 --mate_2_id _p2 --te_fasta %s --genome_fasta %s --fq_dir %s --outdir %s --reference_ins %s --size 500 --step 1234567 --mismatch 2 --run --cpu %s --aligner blat --split --verbose 3' %(RelocaTE, Repeat, Reference, read_dir, outdir, existingTE, cpu)
